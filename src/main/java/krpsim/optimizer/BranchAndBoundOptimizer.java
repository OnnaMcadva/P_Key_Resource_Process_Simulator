...
/**
 * GENERIC Beam Search Optimizer.
 * 
 * Domain-agnostic:
 * - NO hardcoded resource names
 * - NO scenario-specific heuristics
...
public class BranchAndBoundOptimizer implements OptimizationStrategy {
    ...
    private static final int DEFAULT_BEAM_WIDTH = 16;
    ...
    public BranchAndBoundOptimizer(int beamWidth) {
        this.beamWidth = beamWidth;
    }

    @Override
    public String getName() {
        return "Generic Beam Search (width=" + beamWidth + ")";
    }
    ...
    private static final class State {
        ...
        final PriorityQueue<Event> active;
        final List<String> trace;
        int time;
        double heuristic;

        State(Map<String, Integer> stocks,
        ...
        State copy() {
            return new State(stocks, active, trace, time);
        }
    }
    ...
    public OptimizationResult optimize(Parser.Config config, int maxDelay) {
        ...
        beam.add(new State(config.initialStocks(), new PriorityQueue<>(), new ArrayList<>(), 0));
        ...
        while (!beam.isEmpty()) {
            ...
            for (State s : beam) {
                completeFinished(s, processes);

                if (s.time > maxDelay) {
                    double sc = score(s, optimize);
                    if (sc > bestScore) {
                        bestScore = sc;
                        best = s;
                    }
                ...
                List<Process> runnable = runnable(s.stocks, processes);
                ...
                if (!runnable.isEmpty()) {
                    for (Process p : runnable) {
                        ...
                        ns.heuristic = heuristic(ns, optimize, maxDelay);
                        next.add(ns);
                    }
                }
                ...
                if (!s.active.isEmpty()) {
                    State wait = s.copy();
                    wait.time = s.active.peek().time();
                    wait.heuristic = heuristic(wait, optimize, maxDelay);
                    next.add(wait);
                }
            }

            next.sort(Comparator.comparingDouble((State st) -> st.heuristic).reversed());
            beam.clear();
    ...
    /**
     * PURE GENERIC heuristic:
     * - current optimized stocks
     * - remaining time bonus
     */
    private double heuristic(State s, Set<String> optimize, int maxDelay) {
        double v = score(s, optimize);
        int remaining = Math.max(0, maxDelay - s.time);
        return v + remaining * 0.01;
    }

    private double score(State s, Set<String> optimize) {
        double sc = 0;
    ...
    private void completeFinished(State s, List<Process> processes) {
    ...
    private List<Process> runnable(Map<String, Integer> stocks, List<Process> processes) {
